...
import { UseQuestionContext } from "../../contexts/question/QuestionContext";
import { DetailModal } from "./component";

const QuestionPage = () => {
    const { question, deleteQuestion } = UseQuestionContext();
                            ...
                            {(question?.data ?? [])?.map((item, index) => {
                                return (
                                    <tr key={index} className="hover:bg-white-muted">
                                        ...
                                        <td className="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">{item.first_name}</td>
                                        <td className="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">{item.last_name}</td>
                                        <td className="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">{item.email_address}</td>
                                        <td className="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">{item.inquiry_type}</td>
                                        <td className="border-y-[1.5px] border-white-muted px-3 py-3 text-sm text-start">
                                            <div className="flex gap-2 justify-end">
                                                ...
                                                <DeleteConfirmModal onSave={() => deleteQuestion({ id: item.id })}>
                                                    <Button className="rounded-md text-sm border border-dark-muted bg-transparent py-[6px] pl-[10px] pr-[6px] font-normal">
                                                        <span>Delete</span>
