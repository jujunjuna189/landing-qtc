...
import { Button, FieldAsset } from "../../../../components";
import { LinkChecker, updateAssetsApi } from "../../../../helpers";
...
const UpdateModal = (props) => {
    ...
    const onSave = async () => {
        ...
        await updateAssetsApi({ id: props.item?.id, body: dataBatch }).then((res) => {
    ...
    return (
        <div className="inline-block" ref={ref}>
            ...
            {/* Modal */}
            <div className={`fixed top-0 left-0 w-full h-full flex justify-center items-start z-10 transition-opacity duration-300 ${isVisible ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
                {/* Overlay */}
                <div className="absolute w-full h-full bg-dark-dark bg-opacity-75" onClick={toggleModal}></div>

                {/* Modal Content */}
                <div className={`relative p-5 mt-10 border border-white-muted rounded-lg bg-white-light w-[652px] z-20 shadow-all transform transition-transform duration-300 ${isVisible ? "translate-y-0" : "-translate-y-10"}`}>
                    <div className="leading-3">
                        <span className="text-base font-medium">Update Asset</span>
                        <br />
                        {/* <small></small> */}
                    </div>
                            ...
                            <div className="flex gap-5 items-center">
                                <div>
                                    <div className="w-32 h-32 bg-slate-100 flex justify-center items-center relative">
                                        {controller?.file?.type === 'image' ? 
                                            controller?.file?.preview && (<div className="w-full h-full absolute bg-slate-100">
                                                <img src={controller?.file?.preview} alt="ImageProfile" className="object-cover w-full h-full" />
                                            </div>)
                                         : controller?.file?.preview && (
                                            <div className="w-full h-full absolute bg-slate-100">
                                                <video
                                ...
                                <div className="grow">
                                    <div className="mb-2">
                                        <span className="font-medium">Upload Asset</span><br />
                                        <small>Choose a asset and upload it here...</small>
                                    </div>
                                    <div className="flex">
                                        <FieldAsset label="Upload asset" onChange={(value) => onSetController({ field: 'file', value: value })} />
                                        <span className="text-danger-dark">{errors.file}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
